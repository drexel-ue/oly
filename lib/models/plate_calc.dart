...
class PlateSpec {
  ...
  final double
  heightFactor; // 1.0 for standard bumper plate, smaller for fractionals
...
class PlateCalculator {
  ...
  static PlateCalcResult calculate({
    ...
    final double resolvedBar =
        (isLbs && (barWeight == null || barWeight == 20.0))
        ? 45.0
        : (barWeight ?? (isLbs ? 45.0 : 20.0));
    final double resolvedCollar = collarWeight ?? (isLbs ? 0.0 : 2.5);
    final List<PlateSpec> availablePlates = isLbs ? lbsPlates : kgPlates;
    final String unit = isLbs ? 'lbs' : 'kg';
    ...
    final double actualWeight =
        resolvedBar +
        (weightToLoad < 0 ? 0.0 : resolvedCollar) +
        (currentWeight * 2.0);
    final bool isExact = (actualWeight - targetWeight).abs() < 0.01;
