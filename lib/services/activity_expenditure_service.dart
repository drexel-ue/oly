...
class ActivityExpenditureService {
  ...
  static double getMetForLift(String liftId, String exerciseName) {
    final String lId = liftId.toLowerCase();
    final String eName = exerciseName.toLowerCase();
    ...
    if (eName.contains('split squat') ||
        eName.contains('lunge') ||
        eName.contains('hyperextension') ||
        eName.contains('core') ||
        eName.contains('ab')) {
      return 4.5;
    }
  ...
  static DailyActivityEntry createWodActivityEntry({
    ...
    final String name =
        'Olympic Lifting (Day ${session.dayNumber}, Wk ${session.weekNumber})';
