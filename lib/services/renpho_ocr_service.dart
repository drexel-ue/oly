...
class RenphoOcrService {
  ...
  Future<String> extractRawTextFromImage(String imagePath) async {
    final OcrScanResult result = await processImage(imagePath);
  ...
  Future<OcrScanResult> processImage(String imagePath) async {
    try {
      ...
      final BodyCompositionEntry? entry = parseRecognizedText(rawText);
      if (entry != null) {
        ...
        return OcrScanResult(
          rawText: rawText,
          errorMessage:
              'Detected text, but could not identify Renpho scale biometrics.',
          isSuccess: false,
        );
      }
    } catch (e, stack) {
      ...
      AppLogService.instance.error(
        'RENPHO_OCR',
        'OCR Processing exception on $imagePath',
        error: e,
        stackTrace: stack,
      );
      return OcrScanResult(
  ...
  static int _countExtractedFields(BodyCompositionEntry entry) {
    ...
    if (entry.skeletalMuscleLb != null) {
      count++;
    }
    if (entry.fatFreeMassLb != null) {
      count++;
    }
    if (entry.subcutaneousFatPct != null) {
      count++;
    }
    if (entry.visceralFat != null) {
      count++;
    }
    if (entry.bodyWaterLb != null || entry.bodyWaterPct != null) {
      count++;
    }
    if (entry.muscleMassLb != null) {
      count++;
    }
    if (entry.boneMassLb != null) {
      count++;
    }
    if (entry.proteinLb != null || entry.proteinPct != null) {
      count++;
    }
    if (entry.bmrKcal != null) {
      count++;
    }
  ...
  BodyCompositionEntry? parseRecognizedText(String rawText) {
    ...
    final RegExp cluster1Regex = RegExp(
    ...
    final RegExpMatch? c1Match = cluster1Regex.firstMatch(normalized);
    if (c1Match != null) {
    ...
    final RegExp cluster2Regex = RegExp(
    ...
    final RegExpMatch? c2Match = cluster2Regex.firstMatch(normalized);
    if (c2Match != null) {
    ...
    final RegExp cluster4Regex = RegExp(
      r'BMR[\s\S]*?Metabolic\s*Age[\s\S]*?(\d{3,4})\s*(?:kcal)?[\s\n]+(\d{1,3})',
      caseSensitive: false,
    );
    final RegExpMatch? c4Match = cluster4Regex.firstMatch(normalized);
    if (c4Match != null) {
    ...
    if (bodyFatLb == null || bodyFatPct == null) {
      final (double? w, double? p) = extractDual(r'Body\s*Fat');
      bodyFatLb ??= w;
      bodyFatPct ??= p;
    }

    if (skeletalMuscleLb == null || skeletalMusclePct == null) {
      final (double? w, double? p) = extractDual(r'Skeletal\s*Muscle');
      skeletalMuscleLb ??= w;
      skeletalMusclePct ??= p;
    }

    if (bodyWaterLb == null || bodyWaterPct == null) {
      final (double? w, double? p) = extractDual(r'Body\s*Water');
      bodyWaterLb ??= w;
      bodyWaterPct ??= p;
    }

    if (muscleMassLb == null || muscleMassPct == null) {
      final (double? w, double? p) = extractDual(r'Muscle\s*Mass');
      muscleMassLb ??= w;
      muscleMassPct ??= p;
    }

    if (boneMassLb == null || boneMassPct == null) {
      final (double? w, double? p) = extractDual(r'Bone\s*Mass');
      boneMassLb ??= w;
      boneMassPct ??= p;
    }
    ...
    if (subcutaneousFatPct == null) {
      final RegExpMatch? m = RegExp(
        r'Subcutaneous\s*Fat[\s\n]*(?:High|Average|Low|Standard)?[\s\n:]*(\d{1,2}(?:[.,]\d+)?)\s*%',
        caseSensitive: false,
      ).firstMatch(normalized);
      subcutaneousFatPct = double.tryParse(
    ...
    if (weightLb == null || weightLb <= 0) {
      final RegExpMatch? weightDirect = RegExp(
        r'Weight[^\n\r]*?(\d{2,3}(?:[.,]\d+)?)\s*lb',
        caseSensitive: false,
      ).firstMatch(normalized);
      if (weightDirect != null) {
        weightLb = double.tryParse(
          weightDirect.group(1)?.replaceAll(',', '.') ?? '',
        );
      }
    }

    if (weightLb == null || weightLb <= 0) {
      final RegExpMatch? weightFollowed = RegExp(
        r'Weight[\s\n]+(\d{2,3}(?:[.,]\d+)?)',
        caseSensitive: false,
      ).firstMatch(normalized);
      if (weightFollowed != null) {
        weightLb = double.tryParse(
          weightFollowed.group(1)?.replaceAll(',', '.') ?? '',
        );
      }
    }
