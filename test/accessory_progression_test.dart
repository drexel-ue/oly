...
void main() {
  ...
  group('Accessory Weight Progression Storage & Provider Tests', () {
    test('StorageService persists and retrieves accessory logs', () async {
      final history = storage.getAccessoryHistory('bicep_curls');
      ...
      final latest = storage.getLatestAccessoryLog('bicep_curls');
      expect(latest?.weightKg, 17.5);
    });

    test('RecoveryProvider calculates grouped progressions and personal bests', () async {
      final grouped = recoveryProvider.groupedAccessoryProgressions;
      expect(grouped.containsKey('Bicep Curls / Hammer Curls'), isTrue);
      expect(grouped['Bicep Curls / Hammer Curls']!.length, 3);

      final pb = recoveryProvider.getAccessoryPersonalBest('bicep_curls');
      expect(pb, 15.0);
    });
  });

  group('Accessory Progression Widget Tests', () {
    testWidgets('VideoPlayerCard initializes with previous weight and displays PB & History', (tester) async {
      tester.view.physicalSize = const Size(1170, 2532);
      tester.view.devicePixelRatio = 2.0;
      addTearDown(() => tester.view.resetPhysicalSize());

      final accessoryEx = MobilityExerciseModel(
        id: 'bicep_curls',
    ...
    testWidgets('AnalyticsScreen renders Accessories tab with progress cards and deltas', (tester) async {
      tester.view.physicalSize = const Size(1170, 2532);
      tester.view.devicePixelRatio = 2.0;
      addTearDown(() => tester.view.resetPhysicalSize());
