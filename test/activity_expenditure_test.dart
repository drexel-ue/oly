...
void main() {
  group('Activity Expenditure & Metabolic Science Tests', () {
    ...
    test('Demonstrates that Algorithm B prevents 25% overestimation of caloric burn', () {
      ...
      final double pctDifference = ((calA - calB) / calB) * 100.0;
    ...
    test('Maps lift categories to distinct Compendium MET intensities', () {
      expect(
        ActivityExpenditureService.getMetForLift('snatch', 'Power Snatch'),
        equals(6.5),
      );
      expect(
        ActivityExpenditureService.getMetForLift('clean_jerk', 'Clean & Jerk'),
        equals(6.5),
      );
      expect(
        ActivityExpenditureService.getMetForLift(
          'back_squat',
          'Pause Back Squat',
        ),
        equals(6.0),
      ...
      expect(
        ActivityExpenditureService.getMetForLift('clean_pull', 'Clean Pull'),
        equals(6.0),
      );
      expect(
        ActivityExpenditureService.getMetForLift('push_press', 'Push Press'),
        equals(5.0),
      );
      expect(
        ActivityExpenditureService.getMetForLift(
