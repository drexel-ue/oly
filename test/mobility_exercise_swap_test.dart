...
void main() {
  ...
  group('Mobility & Warmup Exercise Swap Tests', () {
    test('MobilitySwapHelper segments exercises into suggested focus/category matches', () {
      final current = MobilityExerciseModel(
        id: 'bicep_curls',
      ...
      final seg = MobilitySwapHelper.segmentExercises(current: current, allExercises: allExercises);

      // Should include overhead tricep extensions (arms focus) and lateral delt flyes (hypertrophyCore) in suggested
      expect(seg.suggested.any((e) => e.name.contains('Tricep')), isTrue);
      // Should not contain itself
      expect(seg.suggested.any((e) => e.id == 'bicep_curls'), isFalse);
      expect(seg.others.any((e) => e.id == 'bicep_curls'), isFalse);
    });

    testWidgets('MobilityExerciseSwapModal displays suggested alternatives and performs swap callback', (tester) async {
    ...
    testWidgets('VideoPlayerCard renders Swap button and triggers swap modal', (tester) async {
      tester.view.physicalSize = const Size(1170, 2532);
      tester.view.devicePixelRatio = 2.0;
      addTearDown(() => tester.view.resetPhysicalSize());
    ...
    testWidgets('WarmupSessionScreen allows swapping exercises and displays SWAPPED status', (tester) async {
      tester.view.physicalSize = const Size(1170, 2532);
      tester.view.devicePixelRatio = 2.0;
      addTearDown(() => tester.view.resetPhysicalSize());
      ...
      await tester.pumpWidget(
        buildTestApp(
          WarmupSessionScreen(dayTemplate: day1),
        ),
      );
    ...
    testWidgets('RecoverySessionScreen allows swapping exercises and resets back', (tester) async {
      tester.view.physicalSize = const Size(1170, 2532);
      tester.view.devicePixelRatio = 2.0;
      addTearDown(() => tester.view.resetPhysicalSize());
