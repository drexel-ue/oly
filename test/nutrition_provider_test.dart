...
void main() {
  ...
  group('Nutrition & BodyComp Provider Tests', () {
    ...
    test('Initializes with baseline Renpho scan', () {
      ...
      expect(bodyCompProvider.latestEntry!.leanBodyMassLb, closeTo(208.6, 0.01));
    });

    test('Logs food entry and calculates correct macro and calorie sums', () async {
      ...
      final log = nutritionProvider.currentDayLog;
    ...
    test('Deletes food entry correctly', () async {
      final item = NutritionEntry.create(
        name: 'Snack Bar',
        calories: 200,
    ...
    test('Adds new Renpho scan and calculates scan deltas', () async {
      final newScan = BodyCompositionEntry.create(
      ...
      expect(bodyCompProvider.weightDeltaVsPrevious, closeTo(-2.3, 0.01)); // -2.3 lb
      expect(bodyCompProvider.bodyFatPctDeltaVsPrevious, closeTo(-0.8, 0.01)); // -0.8%
      expect(bodyCompProvider.leanMassDeltaVsPrevious, greaterThan(0)); // Lean mass preserved/increased!
    });
  });
}
