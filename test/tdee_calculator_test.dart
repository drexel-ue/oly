...
void main() {
  group('TDEE & Macro Calculator Tests', () {
    test('Calculates Katch-McArdle BMR correctly from Lean Mass', () {
      final int bmr = TdeeCalculatorService.calculateKatchMcArdleBmr(208.6);
    ...
    test('Generates athletic macro targets on cutting goal', () {
      ...
      final MacroTargets restDayTargets =
          TdeeCalculatorService.calculateMacroTargets(
            latestBodyComp: entry,
            goal: goal,
            isTrainingDay: false,
          );

      final MacroTargets trainingDayTargets =
          TdeeCalculatorService.calculateMacroTargets(
            latestBodyComp: entry,
            goal: goal,
            isTrainingDay: true,
      ...
      expect(
        trainingDayTargets.calories,
        greaterThan(restDayTargets.calories),
        reason:
            'Rest: ${restDayTargets.calories} (P:${restDayTargets.proteinGrams}, C:${restDayTargets.carbsGrams}, F:${restDayTargets.fatGrams}) vs Train: ${trainingDayTargets.calories} (P:${trainingDayTargets.proteinGrams}, C:${trainingDayTargets.carbsGrams}, F:${trainingDayTargets.fatGrams})',
      );
      expect(
        trainingDayTargets.carbsGrams,
        greaterThan(restDayTargets.carbsGrams),
      );
    });
    ...
    test(
      ...
      () {
        ...
        final int adaptiveTdee = TdeeCalculatorService.calculateAdaptiveTdee(
          recentLogs: logs,
          startComp: startComp,
          endComp: endComp,
        );
    ...
    test('Calculates recommended water intake based on Lean Mass and Training Status', () {
      ...
      final double restDayWater =
          TdeeCalculatorService.calculateRecommendedWaterOz(
            latestBodyComp: entry,
            isTrainingDay: false,
          );
    ...
    test('Applies hydration boost when Renpho Body Water is low (<55%)', () {
      final BodyCompositionEntry dehydratedEntry = BodyCompositionEntry.create(
        weightLb: 264.8,
        fatFreeMassLb: 208.6,
        bodyFatLb: 56.2,
